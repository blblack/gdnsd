...
{
    gdnsd_assume(master_key == NULL); // config only happens once!
...
{
    gdnsd_assume(master_key); // cookie_config() was run
    ...
    bool valid = false;
    const timekeys_t* keys = grcu_dereference(keys_inuse);
